...
<h3>Position Lists</h3>
        <div class="innerLR">
            ...
            <div class="widget">
                <div class="widget-body innerAll inner-2x">
                  <div class="col-sm-5 ">
                   <form  action="{{url('listposition')}}" method="post"  class="form-inline">
                   {{ csrf_field() }}
    ...
    @endforeach 
                    ...
                    <div class="col-sm-5 ad">
                        ...
                        <button  class="btn btn-primary added" data-toggle="modal" data-target="#myModal">Add position</button>
                    </div>
                    <br>
                    <!-- Table -->
...
@if(isset($position))
                                ...
                                @foreach($position as $stu)
                                <tr class="gradeA">

                                   <td>{{ $stu->id }}</td>
                                    <td>{{ $stu->value }}</td>
                                    <td> <input type="submit" value="Edit" class='btn btn-primary  edit' id="edit_{{$stu->id}}"  onclick="edit_row('{{$stu->id}}', 'contact_table');">
                                        <input type='submit' class="btn btn-primary  save" id="save_{{$stu->id}}" value="Save" onclick="save_row('{{$stu->id}}', 'position_table');" style="display:none"></td>   
                                    <td> <input name="remove_levels" type="submit" value="Delete" class='btn btn-primary delete launchConfirm' onclick="delete_row('{{$stu->id}}', 32);" id="delete_{{$stu->id}}" href='#' >
                                        <input type='submit' class="btn btn-primary  cancel" id="cancel_{{$stu->id}}" value="Cancel" onclick="cancel_row('{{$stu->id}}', 'position_table');" style="display:none"></td>  
...
<div id="myModal" class="modal fade" role="dialog" >
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                <form role="form" action="{{url('addPosition')}}" method="post">
                    {{ csrf_field() }}
